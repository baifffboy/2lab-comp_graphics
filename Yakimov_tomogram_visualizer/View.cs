...
namespace Yakimov_tomogram_visualizer
{
    internal class View
    {
        ...
        public void DrawQuads(int layerNumber, bool n)
        {
            if (n == false)
            {
                ...
                for (int x_coord = 0; x_coord < Bin.X - 1; x_coord++)
                {
                    for (int y_coord = 0; y_coord < Bin.Y - 1; y_coord++)
                    {
                        ...
                        value = Bin.array[x_coord + y_coord * Bin.X + layerNumber * Bin.X * Bin.Y];
                        GL.Color3(TransferFunction(value));
                        GL.Vertex2(x_coord, Bin.Y - 1 - y_coord);
                        //2 вершина
                        value = Bin.array[x_coord + (y_coord + 1) * Bin.X + layerNumber * Bin.X * Bin.Y];
                        GL.Color3(TransferFunction(value));
                        GL.Vertex2(x_coord, Bin.Y - 1 - (y_coord + 1)); 
                        //3 вершина
                        value = Bin.array[(x_coord + 1) + (y_coord + 1) * Bin.X + layerNumber * Bin.X * Bin.Y];
                        GL.Color3(TransferFunction(value));
                        GL.Vertex2(x_coord + 1, Bin.Y - 1 - (y_coord + 1)); 
                        //4 вершина
                        value = Bin.array[(x_coord + 1) + y_coord * Bin.X + layerNumber * Bin.X * Bin.Y];
                        GL.Color3(TransferFunction(value));
                        GL.Vertex2(x_coord + 1, Bin.Y - 1 - y_coord); 
                    }
                }
                GL.End();
            }
        ...
        public void Load2DTexture() {
            ...
            data = textureImage.LockBits(new System.Drawing.Rectangle(0, 0, textureImage.Width, textureImage.Height), 
